...
SLOT="0"
IUSE="aix apidoc build +changelog doc gentoo-dev +ipc +native-extensions +pax rsync rsync-verify +sandbox selinux test +verbose xattr"

REQUIRED_USE="
	elibc_glibc? ( sandbox )
	elibc_musl? ( sandbox )
	elibc_uclibc? ( sandbox )
	kernel_AIX? ( pax )
	kernel_Darwin? ( pax )
	kernel_FreeBSD? ( pax )
	kernel_linux? ( pax )
...
RDEPEND="
	...
	sandbox? ( !prefix? ( >=sys-apps/sandbox-2.2 ) )
	kernel_linux? ( sys-apps/util-linux )
	pax? ( >=app-misc/pax-utils-0.1.17 )
