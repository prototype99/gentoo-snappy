...
DESCRIPTION="Meta package to pull in packages and scripts to help keep the world file clean"
...
fm-paste-img? ( pantheon-files )
fm-search? ( pantheon-files pcmanfm-qt dolphin xfe doublecmd ddefm nemo maui-index )
fm-search-contents? ( pcmanfm-qt dolphin xfe doublecmd )
fm-session? ( pantheon-files )
fm-tabs? ( pantheon-files pcmanfm-qt dolphin doublecmd )
...
show-sym? ( pantheon-files pcmanfm-qt )
...
text-editor-color-highlight? ( sublime )
text-editor-js-lint? ( sublimelinter )
...
RDEPEND="
	...
	ddefm? ( dde-base/dde-file-manager )
	...
	irc? ( net-irc/kvirc )
	java-ide? ( >=dev-util/eclipse-sdk-bin-4.13.0 )
	kate? ( kde-plasma/plasma-meta[text-editor] )
	...
	mips-emulate? ( app-emulation/mars-mips )
	...
	pantheon-files? (
		>=pantheon-base/pantheon-files-4.5.0[mod-date-prop?,img-dim-hover-list?,img-dim-jpg?,progress-icon?,thumbnail-jpg?]
	)
	...
	secondlife? ( games-simulation/firestorm-bin )
	spreadsheet? ( app-office/gnumeric )
	sql? ( dev-db/mariadb )
	steam? ( games-util/steam-launcher )
	sublime? ( app-editors/sublime-text )
	sudo? ( app-admin/sudo )
	system-flake8? ( dev-python/flake8 )
	system-pylint? ( >=dev-python/pylint-1.0 )
