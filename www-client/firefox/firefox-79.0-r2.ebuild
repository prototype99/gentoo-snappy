...
src_prepare() {
	...
	eapply "${FILESDIR}/system-dpi.patch"
	...
	eapply "${WORKDIR}/${PN}/0029-bmo-1264836-Automatically-convert-the-little-endian-.patch"
	#hard requires the previous patch
	eapply "${WORKDIR}/${PN}/0030-bmo-1651207-Build-the-ICU-data-through-an-assembly-f.patch"
	eapply "${WORKDIR}/${PN}/0031-Add-support-for-PipeWire-0.3.patch"
	eapply "${WORKDIR}/${PN}/0033-bmo-1645671-Create-DMABufSurfaceWrapper-directly-at-.patch"
	eapply "${WORKDIR}/${PN}/0034-bmo-1656436-Release-VAAPI-surface-data-before-we-re-.patch"
	...
	if use flash; then
		...
		eapply "${FILESDIR}/Don-t-register-plugins-if-the-MOZILLA_DISABLE_PLUGIN.patch"
	fi
	use geckodriver && eapply "${WORKDIR}/${PN}/0032-geckodriver-Build-regex-with-unicode-feature.patch"
	if use kde; then
		sed -e 's:@BINPATH@/defaults/pref/kde.js:@RESPATH@/browser/@PREF_DIR@/kde.js:' \
			"${DISTDIR}/${PN}-${HG_MOZ_PV}-${PN}-kde.patch" > \
	...
	if use lto; then
		...
		eapply "${WORKDIR}/${PN}/0035-bmo-1640982-Set-CARGO_PROFILE_RELEASE_LTO-true-when-.patch"
	...
	! use test && eapply "${FILESDIR}/dont-build-image-gtests-79.patch"
