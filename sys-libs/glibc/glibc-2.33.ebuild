...
SRC_URI+=" https://dev.gentoo.org/~dilfridge/distfiles/${P}-patches-2.tar.xz"

SRC_URI+=" https://gitweb.gentoo.org/proj/locale-gen.git/snapshot/locale-gen-2.10.tar.gz"
SRC_URI+=" multilib? ( https://dev.gentoo.org/~dilfridge/distfiles/gcc-multilib-bootstrap-20201208.tar.xz )"

IUSE="abi_x86_x32 audit caps cet compile-locales +crypt custom-cflags doc gd headers-only +multiarch multilib multilib-bootstrap nscd profile selinux +ssp +static-libs static-pie suid systemtap test vanilla"
...
src_unpack() {
	...
	use multilib-bootstrap && unpack gcc-multilib-bootstrap-20201208.tar.xz
	...
	cd "${WORKDIR}" || die
	unpack ${P}-patches-2.tar.xz

	cd "${WORKDIR}" || die
	unpack locale-gen-2.10.tar.gz
}

src_prepare() {
	if ! use vanilla ; then
		elog "Applying Gentoo Glibc Patchset 2.33-2"
		local PATCHDIR="${WORKDIR}"/patches
		local PATCHES=(
			"${PATCHDIR}"/0001-Gentoo-disable-ldconfig-during-install.patch
			"${PATCHDIR}"/0002-Revert-sysdeps-posix-getaddrinfo.c-gaih_inet-Only-us.patch
			"${PATCHDIR}"/0003-Adapt-to-Gentoo-specific-etc-mail-aliases.patch
			"${PATCHDIR}"/0004-Add-C.UTF-8-locale.patch
			"${PATCHDIR}"/0005-Force-O0-in-conform-tests-to-survive-CC-changes.patch
			"${PATCHDIR}"/0009-x86-Use-SIZE_MAX-instead-of-long-int-1-for-tunable-r.patch
		)
		if use ia64; then
			PATCHES+=(
				"${PATCHDIR}"/0006-Fix-miscompilation-on-ia64-s-gcc-10.patch
			)
		fi
		if use s390; then
			PATCHES+=(
				"${PATCHDIR}"/0011-S390-Add-new-hwcap-values.patch
			)
		fi
		einfo "Done."
	fi
	...
	mkdir extra || die
	mv locale-gen-2.10 extra/locale || die
...
glibc_do_configure() {
	...
	#
	if [[ -n 20201208 ]] && use multilib-bootstrap ; then
		echo 'main(){}' > "${T}"/test.c
		if ! $(tc-getCC ${CTARGET}) ${CFLAGS} ${LDFLAGS} "${T}"/test.c -Wl,-emain -lgcc 2>/dev/null ; then
			sed -i -e '/^CC = /s:$: -B$(objdir)/../'"gcc-multilib-bootstrap-20201208/${ABI}:" config.make || die
		fi
	fi
}
