...
HOMEPAGE="https://www.gnu.org/software/${PN}/ https://invisible-island.net/${PN}/"
SRC_URI="https://github.com/prototype99/${PN}/archive/v6.2.1.tar.gz"
...
#	berkdb? ( sys-libs/db )"
# Block the older ${PN} that installed all files w/SLOT=5. #557472
RDEPEND="${DEPEND}
	!<=sys-libs/${PN}-5.9-r4:5
	!<sys-libs/slang-2.3.2_pre23
	!<x11-terms/rxvt-unicode-9.06-r3
	!<x11-terms/st-0.6-r1"

src_prepare() {
	if use rxvt && use unicode; then
		PATCHES+=(
...
src_configure() {
	...
	# Build the various variants of ${PN} -- narrow, wide, and threaded. #510440
	...
	NCURSES_TARGETS=(
		...
		$(usex unicode '${PN}w' '')
		$(usex threads '${PN}t' '')
		$(use unicode && usex threads '${PN}tw' '')
	)

	# When installing ${PN}, we have to use a compatible version of tic.
...
do_configure() {
	...
	if [[ ${target} == "${PN}" ]] ; then
		# "${PN}" variant goes into "${EPREFIX}"/usr/include
		# It is needed on Prefix because the configure script appends
		# "${PN}" to "${prefix}/include" if "${prefix}" is not /usr.
...
multilib_src_install() {
	...
	if ! tc-is-static-only ; then
		...
		ln -sf lib${PN}$(get_libname) "${ED}"/usr/$(get_libdir)/libcurses$(get_libname) || die
	fi
...
#	fi
	...
	echo "CONFIG_PROTECT_MASK=\"/etc/terminfo\"" | newenvd - "50${PN}"

	use minimal && rm -r "${ED}"/usr/share/terminfo*
	# Because ${PN}5-config --terminfo returns the directory we keep it
...
pkg_preinst() {
	preserve_old_lib "/$(get_libdir)/lib${PN}.so.5"
	use unicode && preserve_old_lib "/$(get_libdir)/lib${PN}w.so.5"
}

pkg_postinst() {
	preserve_old_lib_notify "/$(get_libdir)/lib${PN}.so.5"
	use unicode && preserve_old_lib_notify "/$(get_libdir)/lib${PN}w.so.5"
}
