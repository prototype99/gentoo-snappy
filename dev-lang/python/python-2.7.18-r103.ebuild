...
inherit autotools flag-o-matic pax-utils ${PN}-utils-r1 toolchain-funcs multiprocessing verify-sig
...
src_install() {
	...
	# ${PN}2* is no longer wrapped, so just symlink it
	dosym "${PN}2.7" "/usr/bin/${PN}2"
	dosym "${PN}2.7-config" "/usr/bin/${PN}2-config"
}
